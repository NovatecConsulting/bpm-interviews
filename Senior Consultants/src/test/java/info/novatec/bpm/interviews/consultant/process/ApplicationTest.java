...
import info.novatec.bpm.interviews.senior.application.adapter.in.engine.delegates.ApplicationDenialSender;
import info.novatec.bpm.interviews.senior.application.adapter.in.engine.delegates.ApplicationTester;
import info.novatec.bpm.interviews.senior.application.adapter.in.engine.delegates.InterviewInvitationSender;
import info.novatec.bpm.interviews.senior.application.adapter.in.engine.delegates.JobOfferSender;
...
public class ApplicationTest {
    ...
    public void testHappyPath() {
        ...
        assertThat(processInstance).isWaitingAtExactly("UserTask_BewerbungsgespraechFueren", "UserTask_VertragVorbereiten");
        complete(task("UserTask_VertragVorbereiten"));
        complete(task("UserTask_BewerbungsgespraechFueren"), withVariables("succesful", true));
    ...
    @Test
    public void testManualApplicationDenial() {
        ProcessInstance processInstance = runtimeService.createProcessInstanceByKey(PROCESS_DEFINITION_KEY)
            .startBeforeActivity("Gateway_KandidatGeeignet")
    ...
    @Test
    public void denialAfterBewerbungsgespraech() {
        ProcessInstance processInstance = runtimeService.createProcessInstanceByKey(PROCESS_DEFINITION_KEY)
            .startBeforeActivity("Gateway_GespraechGutGelaufen")
            .setVariable("succesful", false)
            .execute();
